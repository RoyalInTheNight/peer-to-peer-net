...
sigSignalServer::sigSignalServer(std::string nameNode, 
    ...
    Addresses.push_back(addressNode.c_str());
...
void sigSignalServer::sigInit() {
    ...
    nameBuffer = new char[bytes16size];
    ...
    auto dbNode = ([&]() -> int8_t {
        ...
        while (1) {
            ...
            memset(nameBuffer, 0, bytes16size);

            utils_local._recv(fd2, (win_netbuffer)nameBuffer, bytes16size, 0);
